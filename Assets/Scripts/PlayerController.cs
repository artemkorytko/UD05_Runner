...
namespace Runner
{
    public class PlayerController : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            _inputHandler = GetComponentInChildren<InputHandler>();
        ...
        private void Move()
        {
            var xOffseet = -_inputHandler.HorizontalAxis * roadWidth;
            var position = _rigidbody.position;
            position.x += xOffseet;

            var rotation = model.localRotation.eulerAngles;
            rotation.y = Mathf.LerpAngle(rotation.y, xOffseet == 0 ? 0 : Mathf.Sign(xOffseet) * turnRotationAngle,
